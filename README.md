# Microservices with Node, Express and Typescript
...
tsc --init
```
### Make the following changes in the tsconfig file(check the uncommented lines).
~~~
{
  "compilerOptions": {
...
~~~
## Copy the package.json file and run `npm i`
1. The `build` script transpiles the typescript files(starting from the root level) to javascript and places it in the path mentioned in the `outDir` property in the tsconfig file.
2. The `dev` script runs the `ts-node src/app.ts` command to host the code on the local server and watch for the changes if you make any.
3. The `start` script hosts the thus transpiled javascript files placed in the `dist` folder in this case to the local server.
```
{
  "name": "ideaservice",
  "version": "1.0.0",
...
```
## service.ts
Write your bussiness logic here.
```
